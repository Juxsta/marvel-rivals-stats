...
def test_postgres_reachable_from_app():
    ...
    assert (
        conn is not None
    ), "Should be able to connect to PostgreSQL from app container"
...
def test_environment_variables_loaded():
    ...
    assert (
        os.getenv("DATABASE_PASSWORD") is not None
    ), "DATABASE_PASSWORD should be set"
    ...
    ), "PYTHONDONTWRITEBYTECODE should be set to 1"
